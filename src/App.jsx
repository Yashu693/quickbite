...
// ═══════════════════════════════════════════════════════════════════════════
//  STORAGE LAYER  (localStorage now → swap for Firestore later)
// ═══════════════════════════════════════════════════════════════════════════
const DB = {
...
const FOOD = [
  { id:1,  name:'Special Samosa',   cat:'Snacks', price:25,  rating:4.8, votes:'2.3k', time:5,  cal:180, tag:'Bestseller',    veg:true, emoji:'🥟', desc:'Crispy gold pastry stuffed with spiced potato & peas',                 img:'https://images.unsplash.com/photo-1601050690597-df0568f70950?w=400&q=75', g:'linear-gradient(135deg,#f97316,#ef4444)', proof:{orders:47,viewers:3,low:false} },
  { id:2,  name:'Cold Coffee',      cat:'Drinks', price:60,  rating:4.9, votes:'1.8k', time:3,  cal:220, tag:'Fan Fav ❤️',    veg:true, emoji:'☕', desc:'Thick cold brew with chocolate drizzle & whipped cream',               img:'https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=400&q=75', g:'linear-gradient(135deg,#78350f,#1c0a00)', proof:{orders:31,viewers:5,low:false} },
  { id:3,  name:'Vada Pav',         cat:'Snacks', price:20,  rating:4.7, votes:'3.1k', time:4,  cal:150, tag:'Classic',        veg:true, emoji:'🍔', desc:"Mumbai's legendary street burger with green chutney",                  img:'https://images.unsplash.com/photo-1606491956689-2ea866880c84?w=400&q=75', g:'linear-gradient(135deg,#d97706,#92400e)', proof:{orders:62,viewers:2,low:false} },
  { id:4,  name:'Grilled Sandwich', cat:'Combos', price:80,  rating:4.9, votes:'987',  time:8,  cal:380, tag:'🔥 Hot',          veg:true, emoji:'🥪', desc:'Double molten cheddar with fresh veggies on toasted bread',            img:'https://images.unsplash.com/photo-1528736235302-52922df5c122?w=400&q=75', g:'linear-gradient(135deg,#f59e0b,#d97706)', proof:{orders:18,viewers:4,low:true}  },
  { id:5,  name:'Masala Chai',      cat:'Drinks', price:15,  rating:4.6, votes:'4.5k', time:2,  cal:80,  tag:'⚡ Quick',        veg:true, emoji:'🍵', desc:'Aromatic ginger-cardamom brew steeped to perfection',                  img:'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=400&q=75', g:'linear-gradient(135deg,#ef4444,#dc2626)', proof:{orders:88,viewers:6,low:false} },
  { id:6,  name:'Paneer Wrap',      cat:'Combos', price:75,  rating:4.8, votes:'654',  time:7,  cal:320, tag:'Filling',         veg:true, emoji:'🌯', desc:'Creamy tikka paneer stuffed in a soft warm roti wrap',                 img:'https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=400&q=75', g:'linear-gradient(135deg,#22c55e,#16a34a)', proof:{orders:23,viewers:2,low:false} },
  { id:7,  name:'Pav Bhaji',        cat:'Combos', price:55,  rating:4.7, votes:'1.9k', time:10, cal:420, tag:'🌶️ Spicy',        veg:true, emoji:'🍛', desc:'Mumbai-style spicy mashed veggie curry with buttered pav',             img:'https://images.unsplash.com/photo-1606330665884-89cf39fda1bf?w=400&q=75', g:'linear-gradient(135deg,#ef4444,#f97316)', proof:{orders:39,viewers:3,low:false} },
  { id:8,  name:'Fresh Lime Soda',  cat:'Drinks', price:30,  rating:4.5, votes:'789',  time:2,  cal:45,  tag:'Refreshing',      veg:true, emoji:'🥤', desc:'Chilled fizzy lime soda with a perfectly salted rim',                  img:'https://images.unsplash.com/photo-1437418747212-8d9709afab22?w=400&q=75', g:'linear-gradient(135deg,#84cc16,#16a34a)', proof:{orders:14,viewers:1,low:false} },
  { id:9,  name:'Poha',             cat:'Snacks', price:30,  rating:4.6, votes:'1.2k', time:6,  cal:210, tag:'Morning Pick',    veg:true, emoji:'🍚', desc:'Light & fluffy flattened rice with peanuts and curry leaves',           img:'https://images.unsplash.com/photo-1567188040759-fb8a883dc6d6?w=400&q=75', g:'linear-gradient(135deg,#fbbf24,#f59e0b)', proof:{orders:29,viewers:2,low:false} },
  { id:10, name:'Mango Lassi',      cat:'Drinks', price:50,  rating:4.8, votes:'2.1k', time:3,  cal:260, tag:'Summer Love',     veg:true, emoji:'🥭', desc:'Thick creamy mango blended with chilled yogurt & saffron',             img:'https://images.unsplash.com/photo-1571091718767-18b5b1457add?w=400&q=75', g:'linear-gradient(135deg,#f59e0b,#d97706)', proof:{orders:41,viewers:4,low:true}  },
];
...
function useToasts() {
  ...
  const add = useCallback(({ icon, title, msg, cta = '→', type = 'info' }) => {
    const id = Date.now() + Math.random();
    set(p => [{ id, icon, title, msg, cta, type }, ...p.slice(0, 2)]);
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" style={style}>
      <defs>
        ...
        <linearGradient id="qbg2" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%"   stopColor="#fff" stopOpacity=".9" />
          <stop offset="100%" stopColor="#fff" stopOpacity=".7" />
        </linearGradient>
      </defs>
      <rect width="48" height="48" rx="14" fill="url(#qbg1)" />
      <rect width="48" height="48" rx="14" fill="url(#qbg2)" opacity=".06" />
      ...
      <line x1="12" y1="30" x2="36" y2="30" stroke="rgba(255,255,255,.35)" strokeWidth="1" strokeLinecap="round" />
...
function NetworkBanner({ online }) {
  ...
  useEffect(() => {
    ...
    else { setTimeout(() => setShow(false), 2200); }
  }, [online]);
  if (!show) return null;
  return (
    <div style={{
      ...
      padding: '6px 16px', textAlign: 'center', letterSpacing: .3,
      animation: 'netSlide .35s ease',
      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
      backdropFilter: 'blur(12px)',
    }}>
      {online ? '✅ Back online!' : '📡 No internet connection — changes will sync when reconnected'}
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 8999, padding: '0 12px', pointerEvents: 'none' }}>
      {toasts.map((t, i) => (
        <div key={t.id} style={{
          marginTop: i === 0 ? 54 : 6,
          background: 'rgba(6,4,8,.97)', backdropFilter: 'blur(28px) saturate(200%)',
          color: '#fff', borderRadius: 20, padding: '12px 15px',
          display: 'flex', alignItems: 'center', gap: 12,
          boxShadow: '0 20px 60px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.08)',
          border: '1px solid rgba(255,107,53,.2)',
          animation: 'toastSlide .42s cubic-bezier(.34,1.56,.64,1) both',
          pointerEvents: 'auto',
        }}>
          <div style={{ width: 40, height: 40, borderRadius: 13, background: 'linear-gradient(135deg,rgba(255,107,53,.18),rgba(255,61,96,.18))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>{t.icon}</div>
...
const LazyImg = memo(function LazyImg({ item, h = 132 }) {
  ...
  return (
    <div style={{ position: 'relative', height: h, overflow: 'hidden', borderRadius: 'inherit' }}>
      ...
      {s === 'error' ? (
        <div style={{ position: 'absolute', inset: 0, background: item.g, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 54 }}>{item.emoji}</div>
      ) : (
  ...
  return (
    <div className="glass"
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom,transparent 55%,rgba(0,0,0,.42) 100%)', transform: hov ? 'scale(1.06)' : 'scale(1)', transition: 'transform .4s ease', pointerEvents: 'none' }} />
        {item.tag && <div style={{ position: 'absolute', top: 9, left: 9, background: 'rgba(255,255,255,.96)', backdropFilter: 'blur(10px)', color: '#ea580c', fontSize: 9.5, fontWeight: 800, padding: '3px 9px', borderRadius: 99, letterSpacing: .3, transition: 'transform .28s', transform: hov ? 'scale(1.06)' : 'scale(1)' }}>{item.tag}</div>}
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,rgba(255,107,53,.08),rgba(255,61,96,.04))', opacity: hov ? 1 : 0, transition: 'opacity .28s', pointerEvents: 'none' }} />
      </div>
      <div style={{ padding: '11px 12px', background: 'var(--card)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 3 }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--acc)', flexShrink: 0, transition: 'transform .2s', transform: hov ? 'scale(1.1)' : 'scale(1)' }}>₹{item.price}</div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 9 }}>
          ...
          {qty === 0 ? (
            <button onClick={onAdd} className="press" style={{ background: 'linear-gradient(135deg,#FF6B35,#FF3D60)', color: '#fff', border: 'none', borderRadius: 11, padding: '7px 14px', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', boxShadow: hov ? '0 6px 22px rgba(255,107,53,.6)' : '0 4px 16px rgba(255,107,53,.45)', letterSpacing: .3, transition: 'box-shadow .25s', transform: hov ? 'scale(1.04)' : 'scale(1)' }}>+ ADD</button>
          ) : (
...
function BannerCarousel({ onToast }) {
  ...
  const b = BANNERS[idx];
  return (
    <div style={{ borderRadius: 22, overflow: 'hidden', marginBottom: 18, position: 'relative', boxShadow: '0 10px 36px rgba(255,107,53,.28)', cursor: 'pointer' }}
      onClick={() => onToast({ icon: '🏷️', title: 'Coupon copied!', msg: `Use "${b.code}" at checkout`, cta: 'GOT IT' })}>
      <div style={{ background: b.bg, padding: '20px 18px 22px', minHeight: 100, position: 'relative', overflow: 'hidden', transition: 'background .8s ease' }}>
        <div style={{ position: 'absolute', right: -22, top: -22, fontSize: 106, opacity: .11, transform: 'rotate(-16deg)' }}>{b.e}</div>
        <div style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', background: 'rgba(255,255,255,.2)', backdropFilter: 'blur(14px)', borderRadius: 14, padding: '10px 14px', border: '1px solid rgba(255,255,255,.3)' }}>
          <div style={{ fontSize: 10.5, color: '#fff', fontWeight: 800, textAlign: 'center', letterSpacing: .6 }}>{b.cta}</div>
        </div>
        <div style={{ display: 'inline-block', background: 'rgba(255,255,255,.18)', borderRadius: 99, padding: '2px 10px', fontSize: 10, color: '#fff', fontWeight: 800, letterSpacing: .4, marginBottom: 5 }}>{b.code}</div>
        <div style={{ fontSize: 20, fontWeight: 900, color: '#fff', letterSpacing: -.5, lineHeight: 1.1, marginBottom: 5, fontFamily: "'Sora',sans-serif" }}>{b.title}</div>
        <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,.84)', fontWeight: 500 }}>{b.sub}</div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'center', gap: 5, padding: '8px', background: 'rgba(0,0,0,.18)' }}>
        {BANNERS.map((_, i) => (
          <div key={i} onClick={e => { e.stopPropagation(); setIdx(i); }} style={{ width: i === idx ? 22 : 6, height: 6, borderRadius: 99, background: i === idx ? '#fff' : 'rgba(255,255,255,.32)', transition: 'all .35s ease', cursor: 'pointer' }} />
        ))}
      </div>
    </div>
  );
    ...
    <div style={{ position: 'relative', width: 148, height: 148, margin: '0 auto' }}>
      <svg width="148" height="148" viewBox="0 0 148 148">
        <defs>
          ...
          <filter id="ctBlur"><feGaussianBlur stdDeviation="2.5" result="blur" /><feComposite in="SourceGraphic" in2="blur" operator="over" /></filter>
        </defs>
        ...
        {!isReady && progress > 0.02 && progress < 1 && (
          <circle
            ...
            r="5" fill="#FF6B35" filter="url(#ctBlur)" opacity=".85" />
        )}
      </svg>
      ...
      {tabs.map(t => (
        <div key={t.id} onClick={() => go(t.id)} className="press" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', padding: '3px 0' }}>
          <div style={{ position: 'relative' }}>
            ...
            {t.badge > 0 && <div style={{ position: 'absolute', top: -3, right: -3, background: 'linear-gradient(135deg,#FF6B35,#FF3D60)', color: '#fff', fontSize: 8.5, fontWeight: 800, width: 17, height: 17, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(255,107,53,.5)' }}>{t.badge > 9 ? '9+' : t.badge}</div>}
          </div>
        ...
        {tab === 'signup' && (
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input value={name} onChange={e => setName(e.target.value)} onFocus={() => setFN(true)} onBlur={() => setFN(false)} placeholder="Your Name" style={inp(fN)} />
            </div>
          </div>
        )}
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ position: 'relative' }}>
            <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', fontSize: 16 }}>📧</span>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} onFocus={() => setFE(true)} onBlur={() => setFE(false)} placeholder="you@college.edu.in" style={inp(fE)} />
          </div>
        </div>
        {!otpSent ? (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ position: 'relative' }}>
              <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', fontSize: 16 }}>🔒</span>
              <input type={showPass ? 'text' : 'password'} value={pass} onChange={e => setPass(e.target.value)} onFocus={() => setFP(true)} onBlur={() => setFP(false)} onKeyDown={e => e.key === 'Enter' && doLogin()} placeholder="••••••••" style={{ ...inp(fP), paddingRight: 46 }} />
          ...
          <div className="anim-scaleIn" style={{ width: '100%', maxWidth: 340, borderRadius: 24, padding: '20px 0 8px', background: 'var(--card)', border: '1px solid var(--bdr)' }}>
            <div style={{ padding: '0 20px 16px', borderBottom: '1px solid var(--bdr)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--txt)', fontFamily: "'Sora',sans-serif" }}>Sign in with Google</div>
              </div>
            ...
            {MOCK_ACCOUNTS.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--mut)', marginBottom: 14 }}>Enter your Google account email to continue</div>
                <input placeholder="your@gmail.com" style={{ width: '100%', padding: '11px 14px', borderRadius: 12, border: '2px solid var(--inpB)', background: 'var(--inp)', fontSize: 13, color: 'var(--txt)', marginBottom: 10 }}
                  onKeyDown={e => { if (e.key === 'Enter' && e.target.value.includes('@')) { const nm = e.target.value.split('@')[0]; googleLogin({ name: nm.charAt(0).toUpperCase() + nm.slice(1), email: e.target.value, avatar: Math.floor(Math.random() * 5) }); } }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 11, marginBottom: 22 }}>
          {COLLEGES.map((c, i) => (
            <div key={c.id} className={`anim-fadeUp ${c.locked ? '' : 'press'}`}
              ...
              {c.locked && (
                <div style={{ position: 'absolute', inset: 0, borderRadius: 18, background: 'rgba(0,0,0,.04)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', paddingBottom: 10, zIndex: 2, pointerEvents: 'none' }}>
...
//  HOME VIEW
// ═══════════════════════════════════════════════════════════════════════════
function HomeView({ user, college, cart, setCart, addToast, go, dark, toggleDark, skeletonMode, menuData, loadingMenu }) {
  ...
  const addItem = item => { setCart(p => ({ ...p, [item.id]: (p[item.id] || 0) + 1 })); addToast({ icon: item.emoji, title: 'Added to cart!', msg: `${item.name} · ₹${item.price}`, cta: '🛒' }); };
  const incItem = item => setCart(p => ({ ...p, [item.id]: (p[item.id] || 0) + 1 }));
  const decItem = item => setCart(p => { const n = { ...p }; n[item.id] = (n[item.id] || 1) - 1; if (n[item.id] <= 0) delete n[item.id]; return n; });
  const getQty  = id => cart[id] || 0;

  const aiRecs = useMemo(() => {
    const h = new Date().getHours(), preferSnacks = h < 11 || h > 20;
    return menuData.filter(f => !cart[f.id] && (preferSnacks ? f.cat === 'Snacks' || f.cat === 'Drinks' : true)).sort((a, b) => b.rating - a.rating).slice(0, 4);
  }, [cart, menuData]);

  let filtered = menuData.filter(f => {
  ...
  const totalPrice  = Object.entries(cart).reduce((s, [id, q]) => { const f = menuData.find(x => String(x.id) === String(id)); return s + (f ? f.price * q : 0); }, 0);
  const hdr         = scrollY > 16;
  const hour        = new Date().getHours();
  const greeting    = hour < 12 ? 'Good Morning ☀️' : hour < 17 ? 'Good Afternoon 🌤️' : 'Good Evening 🌙';
  
  const aiCopy = useMemo(() => { const h = new Date().getHours(); if (h < 11) return 'Looks like breakfast time! 🌅'; if (h < 14) return 'Lunch rush — order early! ⚡'; if (h < 17) return 'Afternoon snack time 😋'; return 'Evening munchies incoming 🌙'; }, []);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg)' }}>
      <div className={hdr ? 'glass-hdr' : ''} style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20, padding: '38px 16px 12px', transition: 'all .3s ease', background: hdr ? 'var(--hdr)' : 'transparent' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 9.5, color: 'var(--acc)', fontWeight: 700, letterSpacing: .5, marginBottom: 2, display: 'flex', alignItems: 'center', gap: 4 }}><span>📍</span>{college?.canteen || 'Campus Canteen'}</div>
      ...
      <div ref={scrollRef} className="hs" style={{ position: 'absolute', inset: 0, overflowY: 'auto', overflowX: 'hidden', paddingTop: 90, paddingBottom: totalItems > 0 ? 164 : 88 }}>
        <div style={{ padding: '0 14px' }}>
          
          {/* Upload button hidden so you don't duplicate items! 
          <div style={{ paddingBottom: "14px" }}>
            <button 
              onClick={addAllItemsToFirebase}
              style={{
                width: "100%", padding: "14px", background: "linear-gradient(135deg, #10B981, #059669)",
                color: "white", borderRadius: "14px", border: "none", fontWeight: "800", fontSize: "14px", cursor: "pointer"
              }}>🔥 Upload Entire Menu to Firebase 🚀
            </button>
          </div>
          */}
          ...
          {!search && cat === 'All' && (
            <div className="anim-fadeUp glass" style={{ borderRadius: 16, padding: '11px 14px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10, animationDelay: '.08s' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--txt)' }}>{aiCopy}</div>
          ...
          <div style={{ display: 'flex', gap: 9, marginBottom: 14 }}>
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search samosa, chai..."
                style={{ width: '100%', padding: '12px 36px 12px 40px', borderRadius: 14, border: '2px solid var(--inpB)', background: 'var(--inp)', fontSize: 13, color: 'var(--txt)', transition: 'border-color .2s,box-shadow .2s', backdropFilter: 'blur(10px)' }}
              ...
              {search && <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 11, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: 'var(--mut)', padding: 2 }}>✕</button>}
            </div>
          ...
          {!search && cat === 'All' && aiRecs.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 11, overflowX: 'auto', paddingBottom: 4 }} className="hs">
                {aiRecs.map(item => (
                  <div key={item.id} className="lift glass" style={{ flexShrink: 0, width: 160, borderRadius: 18, overflow: 'hidden' }}>
                    <div style={{ position: 'relative' }}>
                      <LazyImg item={item} h={90} />
                      ...
                      <div style={{ position: 'absolute', top: 6, left: 6, background: 'rgba(124,58,237,.9)', backdropFilter: 'blur(8px)', color: '#fff', fontSize: 8, fontWeight: 800, padding: '2px 7px', borderRadius: 99 }}>🧠 AI Pick</div>
                    </div>
                    <div style={{ padding: '9px 11px', background: 'var(--card)' }}>
                      <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--txt)', marginBottom: 6, fontFamily: "'Sora',sans-serif", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ fontSize: 13.5, fontWeight: 800, color: 'var(--acc)' }}>₹{item.price}</div>
                        <button onClick={() => addItem(item)} className="press" style={{ background: 'linear-gradient(135deg,#FF6B35,#FF3D60)', color: '#fff', border: 'none', borderRadius: 9, padding: '5px 11px', fontSize: 10.5, fontWeight: 800, cursor: 'pointer', boxShadow: '0 3px 10px rgba(255,107,53,.42)' }}>+ ADD</button>
                      </div>
                    </div>
                  </div>
                ))}
...
//  CART VIEW
// ═══════════════════════════════════════════════════════════════════════════
function CartView({ user, college, cart, setCart, addToast, go, onGoToPay, menuData }) {
  const [coupon,    setCoupon]    = useState('');
  const [couponMsg, setCouponMsg] = useState('');
  const [disc,      setDisc]      = useState(0);

  const items      = Object.entries(cart).map(([id, qty]) => { const f = menuData.find(x => String(x.id) === String(id)); return f ? { ...f, qty } : null; }).filter(Boolean);
  ...
  if (items.length === 0) return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 28px' }}>
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg)' }}>
      <div className="glass-hdr" style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20, padding: '38px 16px 14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="glass" style={{ fontSize: 11, color: 'var(--sub)', fontWeight: 600, padding: '5px 12px', borderRadius: 99 }}>{totalItems} item{totalItems !== 1 ? 's' : ''}</div>
        </div>
      </div>
      <div className="hs" style={{ position: 'absolute', top: 90, bottom: 74, overflowY: 'auto', padding: '14px' }}>
        <div className="glass" style={{ borderRadius: 22, overflow: 'hidden', marginBottom: 12 }}>
          <div style={{ padding: '13px 16px', borderBottom: '1px solid var(--bdr)', display: 'flex', gap: 7, alignItems: 'center', background: 'var(--card)' }}>
            <span style={{ fontSize: 16 }}>🛒</span><span style={{ fontSize: 13, fontWeight: 800, color: 'var(--txt)', fontFamily: "'Sora',sans-serif" }}>Order Items</span>
          </div>
          {items.map((item, i) => (
            <div key={item.id} style={{ padding: '13px 16px', borderBottom: i < items.length - 1 ? '1px solid var(--bdr)' : 'none', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--card)' }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: item.g, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{item.emoji}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', background: 'linear-gradient(135deg,#FF6B35,#FF3D60)', borderRadius: 11, overflow: 'hidden', boxShadow: '0 3px 12px rgba(255,107,53,.35)', flexShrink: 0 }}>
                <button onClick={() => dec(item.id)} className="press" style={{ background: 'transparent', border: 'none', color: '#fff', width: 28, height: 28, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                <span style={{ color: '#fff', fontWeight: 800, fontSize: 12, minWidth: 14, textAlign: 'center' }}>{item.qty}</span>
                <button onClick={() => inc(item.id)} className="press" style={{ background: 'transparent', border: 'none', color: '#fff', width: 28, height: 28, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
              </div>
              <div style={{ fontSize: 13.5, fontWeight: 800, color: 'var(--txt)', minWidth: 48, textAlign: 'right', flexShrink: 0 }}>₹{item.price * item.qty}</div>
            </div>
          ))}
        </div>
        <div className="glass" style={{ borderRadius: 18, padding: '14px 16px', marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--txt)', marginBottom: 10, display: 'flex', gap: 6, fontFamily: "'Sora',sans-serif" }}>🏷️ Apply Coupon</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={coupon} onChange={e => setCoupon(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && applyCoupon()} placeholder="Enter code..."
              style={{ flex: 1, padding: '10px 13px', borderRadius: 12, border: '2px solid var(--inpB)', background: 'var(--inp)', fontSize: 12.5, color: 'var(--txt)', letterSpacing: 1.2, fontWeight: 700 }} />
            <button onClick={applyCoupon} className="press" style={{ padding: '10px 16px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#FF6B35,#FF3D60)', color: '#fff', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', boxShadow: '0 4px 14px rgba(255,107,53,.38)' }}>Apply</button>
          </div>
          {couponMsg && <div className="anim-fadeIn" style={{ marginTop: 9, fontSize: 11, color: couponMsg.startsWith('✅') ? 'var(--grn)' : '#ef4444', fontWeight: 700 }}>{couponMsg}</div>}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
            {Object.keys(COUPONS).map(c => <div key={c} onClick={() => setCoupon(c)} className="press" style={{ padding: '4px 10px', borderRadius: 99, border: '1.5px dashed var(--acc)', fontSize: 9.5, fontWeight: 800, color: 'var(--acc)', cursor: 'pointer', letterSpacing: .8 }}>{c}</div>)}
          </div>
        </div>
        <div className="glass" style={{ borderRadius: 18, padding: '14px 16px', marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--txt)', marginBottom: 13, fontFamily: "'Sora',sans-serif" }}>🧾 Bill Summary</div>
          {[['Item Total', `₹${sub}`], ['Service Fee', `₹${PREP_FEE}`]].map(([l, v]) => (
            <div key={l} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 9 }}>
              <span style={{ fontSize: 12.5, color: 'var(--sub)' }}>{l}</span>
              <span style={{ fontSize: 12.5, color: 'var(--txt)', fontWeight: 600 }}>{v}</span>
            </div>
          ))}
          {dk > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 9 }}><span style={{ fontSize: 12.5, color: 'var(--grn)' }}>Coupon Discount</span><span style={{ fontSize: 12.5, color: 'var(--grn)', fontWeight: 700 }}>−₹{dk}</span></div>}
          <div style={{ borderTop: '2px dashed var(--bdr)', paddingTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--txt)', fontFamily: "'Sora',sans-serif" }}>Grand Total</span>
            <span style={{ fontSize: 21, fontWeight: 900, background: 'linear-gradient(135deg,#FF6B35,#FF3D60)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>₹{total}</span>
          </div>
        </div>
        <div style={{ background: 'rgba(34,197,94,.07)', border: '1px solid rgba(34,197,94,.18)', borderRadius: 15, padding: '11px 14px', display: 'flex', gap: 10, alignItems: 'center', marginBottom: 16 }}>
          <span style={{ fontSize: 18 }}>🎉</span>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--grn)' }}>No hidden charges!</div>
            <div style={{ fontSize: 10.5, color: 'var(--grn)', marginTop: 1 }}>All taxes included · Free canteen pickup</div>
          </div>
        </div>
        <SwipeToPay total={total} onPay={() => onGoToPay({ items, sub, fee: PREP_FEE, dk, total })} />
        <div style={{ textAlign: 'center', margin: '12px 0 8px', fontSize: 11.5, color: 'var(--mut)' }}>— or choose payment method —</div>
        <button onClick={() => onGoToPay({ items, sub, fee: PREP_FEE, dk, total })} className="press" style={{ width: '100%', padding: '15px', borderRadius: 17, border: 'none', background: 'linear-gradient(135deg,#1e40af,#1d4ed8)', color: '#fff', fontSize: 13.5, fontWeight: 800, cursor: 'pointer', boxShadow: '0 8px 26px rgba(59,130,246,.38)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginBottom: 26 }}>
      ...
      <div className="hs" style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        {state === 'methods' && (
          <>
            <div className="anim-fadeIn glass" style={{ borderRadius: 18, padding: '14px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ marginLeft: 'auto', fontSize: 22, fontWeight: 900, color: 'var(--acc)', fontFamily: "'Sora',sans-serif" }}>₹{total}</div>
            </div>
        ...
        {state === 'processing' && (
          <div className="anim-fadeIn" style={{ textAlign: 'center', padding: '40px 20px' }}>
            <div style={{ width: 84, height: 84, borderRadius: '50%', background: 'linear-gradient(135deg,rgba(255,107,53,.12),rgba(255,61,96,.08))', margin: '0 auto 22px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function TrackingView({ order, onDone, onViewReceipt, addToast }) {
  ...
  useEffect(() => {
    if (stage < 2) return;
    if (countdown <= 0) {
      if (!notified) { setNotif(true); setDoneIn(elapsed); setStage(3); addToast?.({ icon: '🎯', title: 'Order Ready!', msg: `${order?.token || ''} — Counter B · Pick it up!`, cta: 'COLLECT' }); }
      return;
    }
    const t = setInterval(() => setCd(c => {
      if (c <= 1) {
        setDoneIn(elapsed + 1); setStage(3);
        if (!notified) { setNotif(true); addToast?.({ icon: '🎯', title: 'Order Ready!', msg: `${order?.token || ''} — Counter B · Pick it up!`, cta: 'COLLECT' }); }
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="hs" style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        <div className="print-card anim-slideLeft" style={{ background: 'var(--card)', borderRadius: 24, overflow: 'hidden', border: '1px solid var(--bdr)', boxShadow: '0 8px 40px var(--shadow)' }}>
          ...
          <div style={{ border: 'none', borderTop: '2px dashed var(--bdr)', margin: '0 16px' }} />
      ...
      <div className="hs" style={{ position: 'absolute', top: 88, bottom: 74, overflowY: 'auto', padding: '14px' }}>
        {orders.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--mut)', marginBottom: 26 }}>Your order history appears here after you place your first order!</div>
            ...
            {orders.map((order, idx) => (
              <div key={order.id} className="anim-fadeUp lift glass" style={{ animationDelay: `${idx * .05}s`, borderRadius: 22, overflow: 'hidden', marginBottom: 12 }}>
                ...
                <div style={{ padding: '13px 16px' }}>
                  ...
                  <div className="glass" style={{ borderRadius: 12, padding: '9px 13px', marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
...
function ProfileView({ user, college, dark, toggleDark, go, orders, onLogout, onChangeCollege, cartCount = 0 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg)' }}>
      ...
      <div className="hs" style={{ position: 'absolute', top: 88, bottom: 74, overflowY: 'auto', padding: '14px' }}>
        {/* Profile card */}
        <div className="anim-fadeUp glass" style={{ borderRadius: 24, overflow: 'hidden', marginBottom: 14 }}>
          <div style={{ background: 'linear-gradient(135deg,rgba(255,107,53,.15),rgba(255,61,96,.08))', padding: '24px 20px 20px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', right: -18, top: -18, fontSize: 90, opacity: .07 }}>👤</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 68, height: 68, borderRadius: 22, background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 900, color: '#fff', boxShadow: '0 8px 24px rgba(0,0,0,.22)', flexShrink: 0, fontFamily: "'Sora',sans-serif" }}>{user?.name?.[0] || 'U'}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 20, fontWeight: 900, color: 'var(--txt)', fontFamily: "'Sora',sans-serif", letterSpacing: -.4 }}>{user?.name || 'Student'}</div>
                <div style={{ fontSize: 12, color: 'var(--sub)', marginTop: 3 }}>{user?.email || 'student@college.edu.in'}</div>
                <div style={{ display: 'flex', gap: 6, marginTop: 7 }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(255,107,53,.1)', border: '1px solid rgba(255,107,53,.2)', borderRadius: 99, padding: '3px 10px' }}>
                    <span style={{ fontSize: 11 }}>{college?.emoji || '🎓'}</span>
                    <span style={{ fontSize: 10.5, fontWeight: 700, color: '#ea580c' }}>{college?.short || 'Campus'}</span>
                  </div>
                  {user?.method && <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'var(--inp)', border: '1px solid var(--bdr)', borderRadius: 99, padding: '3px 10px' }}>
                    <span style={{ fontSize: 9.5, fontWeight: 700, color: 'var(--sub)' }}>{user.method === 'google' ? '🔵 Google' : user.method === 'otp' ? '📱 OTP' : '📧 Email'}</span>
                  </div>}
                </div>
              </div>
            </div>
          </div>
          <div style={{ padding: '14px 20px', borderTop: '1px solid var(--bdr)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 22 }}>{college?.emoji || '🏫'}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--txt)', fontFamily: "'Sora',sans-serif" }}>{college?.canteen || 'Campus Canteen'}</div>
              <div style={{ fontSize: 10.5, color: 'var(--mut)', marginTop: 1 }}>{college?.name || 'Your College'} · {college?.area}</div>
            </div>
            <button onClick={onChangeCollege} className="press glass" style={{ padding: '6px 13px', borderRadius: 10, border: '1px solid var(--bdr)', color: 'var(--sub)', fontSize: 10.5, fontWeight: 700, cursor: 'pointer', background: 'transparent', backdropFilter: 'blur(10px)' }}>Change</button>
          </div>
        </div>
        {/* Stats */}
        <div className="anim-fadeUp" style={{ animationDelay: '.06s', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
          {[[orders.length, 'Orders', '📋'], [`₹${totalSpent}`, 'Spent', '💰'], [`${orders.length * 6 + 10}pts`, 'Points', '⭐']].map(([v, l, e]) => (
            <div key={l} onClick={() => l === 'Orders' && go('history')} className="press glass" style={{ borderRadius: 18, padding: '14px 10px', textAlign: 'center', cursor: l === 'Orders' ? 'pointer' : 'default' }}>
              <div style={{ fontSize: 19, marginBottom: 5 }}>{e}</div>
              <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--acc)', fontFamily: "'Sora',sans-serif" }}>{v}</div>
              <div style={{ fontSize: 9.5, color: 'var(--mut)', fontWeight: 600, letterSpacing: .4, textTransform: 'uppercase', marginTop: 3 }}>{l}</div>
            </div>
          ))}
        </div>
        {/* Settings */}
        <div className="anim-fadeUp glass" style={{ animationDelay: '.1s', borderRadius: 20, overflow: 'hidden', marginBottom: 14 }}>
        ...
        {/* Logout */}
        <div className="anim-fadeUp" style={{ animationDelay: '.18s' }}>
          <div style={{ textAlign: 'center', marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: 'var(--mut)', fontWeight: 600 }}>QuickBite v2.0 · Firebase Ready</div>
            <div style={{ fontSize: 10, color: 'var(--mut)', marginTop: 2 }}>{FIREBASE_CONFIG ? '🟢 Firebase Active' : '🟡 localStorage mode'}</div>
          </div>
          <button onClick={onLogout} className="press" style={{ width: '100%', padding: '15px', borderRadius: 17, border: '2px solid rgba(239,68,68,.25)', background: 'rgba(239,68,68,.06)', color: '#ef4444', fontSize: 14, fontWeight: 800, cursor: 'pointer', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9 }}>
            🚪 Log Out
          </button>
        </div>
      </div>
          ...
          <div className="anim-slideUp" style={{ width: '100%', background: 'var(--card)', borderRadius: '28px 28px 0 0', padding: '0 0 36px', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            {modal === 'privacy' && (
              <div style={{ padding: '20px 20px 0' }}>
                ...
                {[{ icon: '🔐', title: 'End-to-End Encryption', desc: 'All payments are encrypted with 256-bit SSL' }, { icon: '🗄️', title: 'Data Storage', desc: 'Your data is stored locally + optionally on Firebase' }, { icon: '🚫', title: 'No Ads or Tracking', desc: 'We never sell your data to third parties' }, { icon: '🗑️', title: 'Delete Account', desc: 'Permanently remove all your data', danger: true }].map((p, i) => (
...
export default function App() {
  ...
  // ── FETCH MENU FROM FIREBASE ON LOAD ──
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        // If Firebase is empty (or offline rules block it), fall back to hardcoded FOOD
        setMenuData(items.length > 0 ? items : FOOD);
      } catch (error) {
        console.error("Error fetching menu:", error);
        setMenuData(FOOD); // Fallback so app doesn't break
      } finally {
  ...
  return (
    <div className={dark ? 'theme-dark' : 'theme-light'} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%', background: dark ? '#06040A' : '#C8B5B5' }}>
      <div style={{ position: 'relative', width: '100%', maxWidth: 430, height: '100%', maxHeight: '100vh', background: 'var(--bg-raw)', overflow: 'hidden', boxShadow: '0 0 0 1px rgba(255,255,255,.04), 0 40px 120px rgba(0,0,0,.65)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%' }}>
          ...
          {/* Passed menuData and loadingMenu down to Home and Cart */}
          {!splash && view === 'home'          && <HomeView          user={user}         college={college} cart={cart} setCart={setCart} addToast={addToast} go={go} dark={dark} toggleDark={toggleDark} skeletonMode={skel} menuData={menuData} loadingMenu={loadingMenu} />}
          {!splash && view === 'cart'          && <CartView          user={user}         college={college} cart={cart} setCart={setCart} addToast={addToast} go={go} onGoToPay={onGoToPay} menuData={menuData} />}
